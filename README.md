...
To test that your node is working as expected, run the following command:

`docker exec bitcoin-core /bin/bash -c "/scripts/bitcoin-cli.sh getblockcount"`

This will ask your node what the latest block number is. You can compare it to the latest block on a 
blockchain explorer like https://mempool.space/. Most likely the number you have will be lower, as the node has to sync
and catch up to the latest blocks.

Run the command multiple times to see the block number increase.

Note that you might have to wait a few minutes before running this test command, as the initial blockchain snapshot 
has to be downloaded first. It is a 5GB archive, so you might want to get a coffee or a steak and then run the above command.

If you see logs similar to the ones below (after running `docker logs bitcoin-core`), that means the node is working
correctly and downloading the most recent blocks.

![Example logs showing that the newest blocks are being downloaded](documentation/ExampleLogs.jpg)

#### Running more commands

The `/scripts/bitcoin-cli.sh` script is a simple wrapper script for the default `bitcoin-cli` binary provided by
[Bitcoin Core](https://github.com/bitcoin/bitcoin), which means you can use any of the commands listed 
[here](https://developer.bitcoin.org/reference/rpc/) or simply run

`docker exec bitcoin-core /bin/bash -c "/scripts/bitcoin-cli.sh help"`

to get the help page for all the available commands.
...
If you have Bitcoin Core installed on your host, you can try connecting using bitcoin-cli (with the appropriate RPC 
User and Password provided) or, you can simply test the connection using CURL like below

`curl --user satoshi --data-binary '{"jsonrpc": "1.0", "id": "curltest", "method": "getblockcount", "params": []}' -H 'content-type: text/plain;' http://127.0.0.1:8332/`

When prompted, enter the password you found in the logs, and you should be given an output similar to the one below, 
with the block number of the latest block in your node

![Example CURL blockcount output](documentation/CurlBlockcount.png)
...
## A Note about Trade-offs

### Pruned node

Running a pruned node doesn't allow you to propagate the blockchain to the network as well.

Normally, a full-block node will have all the blocks stored on disk, meaning that whenever a node joins the network and 
needs to know all the historical blocks, the full-block node can send a copy to that new node. A pruned node cannot do
this because it simply does not have all the blocks, by definition.

A pruned node still verifies all the incoming blocks, but keeps only a summary of what was in the blocks. This allows
the pruned node to use very little disk space, however it means other nodes cannot rely on it for historical data.

### Using a blockchain snapshot

This node uses https://prunednode.today/ to fetch a recent snapshot of a pruned blockchain, in order for you to be able
to quickly use this node.

The standard way is to start the node and wait for it to download all (almost 800,000) historical blocks from other nodes.
(As mentioned above, pruned nodes still verify every single block, they just don't store them on disk forever)

Downloading the whole blockchain from other nodes takes weeks if not months, which is obviously quite problematic.

Using [prunednode.today](https://prunednode.today/) however, means we have to trust the owners of that site that the
blockchain snapshot they put up has not been tampered with.

The node will verify all later blocks (after the snapshot) by itself, so if you are setting up this node to use with a
new wallet, there is very little trust placed in the snapshot.
...
<img src="documentation/QRCode.jpeg" width="200" height="200" />
